...
export default function KnowledgeBase() {
  const { user } = useAuth();
  const canEdit = can(user, "knowledge.edit");
  const canDelete = can(user, "knowledge.delete");
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto" data-testid="kb-page">
      <div className="flex items-baseline justify-between mb-6">
        ...
        {canEdit && (
        <button onClick={() => setOpen(true)} data-testid="add-doc-btn" className="inline-flex items-center gap-2 btn-solid px-4 py-2 text-sm font-semibold">
          <Plus size={14} /> Add document
        </button>
        )}
      </div>

      <form onSubmit={search} className="panel flex items-center mb-6">
        ...
        <input
          ...
          className="flex-1 px-3 py-3 text-sm bg-transparent text-[var(--text-primary)] outline-none"
        />
        <button className="btn-primary px-5 py-3 text-sm font-semibold">Search</button>
      </form>

      {results && (
        <div className="mb-6 panel border-[var(--brand-primary)]">
        ...
        {docs.map((d) => (
          <div key={d.id} className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-[var(--border)] text-sm items-center hover:bg-[var(--bg-soft)]">
            ...
            <span className="col-span-2">
              <span className="inline-flex items-center gap-1.5 text-xs font-mono text-[var(--text-primary)]">
                <span className="w-2 h-2 bg-[var(--brand-success)]" /> {d.status}
              </span>
            </span>
            <span className="col-span-2 text-right font-mono text-xs text-[var(--text-primary)]">{d.chunk_count}</span>
            {canDelete && (
            <button
              onClick={async () => {
...
function AddDocModal({ onClose, onDone }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <form onSubmit={submit} className="modal-shell w-full max-w-2xl" data-testid="add-doc-modal">
        <div className="px-5 py-3 border-b border-[var(--border)] label-mono modal-header">Add document</div>
        <div className="p-5 space-y-4">
          <div>
            <label className="label-mono block mb-1">Title</label>
            <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} data-testid="doc-title" className="field-input" />
          </div>
          <div>
            <label className="label-mono block mb-1">Type</label>
            <select value={form.source_type} onChange={(e) => setForm({ ...form, source_type: e.target.value })} className="field-input">
          ...
          <div>
            ...
            <textarea
              ...
              className="field-input font-mono resize-y"
            />
          </div>
        </div>
        <div className="px-5 py-3 border-t border-[var(--border)] flex justify-end gap-2">
          ...
          <button data-testid="doc-submit" disabled={loading} className="px-4 py-2 text-sm btn-primary">
